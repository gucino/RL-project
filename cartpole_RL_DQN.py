# -*- coding: utf-8 -*-
"""
Created on Sat Apr 18 11:19:59 2020
...
num_hidden1_Q=40
num_hidden2_Q=40
num_output_Q=env.action_space.n
to_render=5000
######################################
######################################
######################################
...
for each_episode in range(0,num_episode):
    ...
    while done==False:
        ...
        if rand<epsilon:
            #take radnom action
            action_to_take=np.random.choice(np.arange(0,num_output_Q))
        else:
        ...
        if len(replay_buffer)>=num_sample_to_draw:
            ...
            for i in range(0,len(sample_list)):
                ...
                #adjust Q value output respects to target value
...
#plot
plt.figure()
plt.title("reward vs episode (cartpole)")
plt.plot(np.arange(0,len(reward_list)),reward_list)
plt.xlabel("num episode")
plt.ylabel("reward")
plt.show()
plt.savefig("reward vs episode (cartpole)")

num_interval=200
smooth=smmooth(reward_list,num_interval)
plt.figure()
plt.title("reward vs episode (cartpole) (smoothed)")
plt.plot(np.arange(0,len(smooth)),smooth)
plt.xlabel("num episode")
plt.ylabel("reward")
plt.show()
plt.savefig("reward vs episode (cartpole) (smoothed)")
